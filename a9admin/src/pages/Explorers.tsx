...
const sortExplorers = (explorers: UserData[], sortConfig: SortConfig) => {
  if (!sortConfig.key) return explorers;
  ...
  return [...explorers].sort((a, b) => {
    ...
    switch (sortConfig.key) {
      ...
      case "registration_date":
        // Date sorting - use created_at for explorers
...
const Explorers: React.FC = () => {
  ...
  const loadExplorers = async () => {
    ...
    try {
      ...
      if (response?.success && Array.isArray(response.data)) {
        console.log("Explorers data:", response.data); // Debug log
  ...
  useEffect(() => {
    ...
    if (filters.jobTitle) {
      result = result.filter((e) =>
        e.job_title?.toLowerCase().includes(filters.jobTitle.toLowerCase())
      );
    }

    if (filters.registrationDate) {
      result = result.filter((e) => {
        const dateField = e.created_at || e.registration_date;
        const formattedDate = formatDate(dateField);
        return formattedDate
          .toLowerCase()
          .includes(filters.registrationDate.toLowerCase());
      });
    }
    ...
    setFiltered(result);
  }, [searchTerm, explorers, filters]);

  // Apply sorting
  useEffect(() => {
    const sortedExplorers = sortExplorers(filtered, sortConfig);
    setSorted(sortedExplorers);
  }, [filtered, sortConfig]);
  ...
  // Get the actual date field for display
  const getDisplayDate = (user: UserData) => {
    return user.created_at || user.registration_date;
  };
            ...
            <div style={styles.searchContainer}>
              ...
              {searchTerm && (
                <span style={styles.resultsText}>
                  Showing {sorted.length} of {explorers.length} explorers
                </span>
              )}
            </div>
          </div>
            ...
            <div style={styles.filterPanel}>
              <div style={styles.filterGrid}>
                <div style={styles.filterGroup}>
                  <label style={styles.filterLabel}>Full Name</label>
                  <input
                    type="text"
                ...
                <div style={styles.filterGroup}>
                  <label style={styles.filterLabel}>Job Title</label>
                  <input
                    type="text"
                  ...
                  <tr style={styles.tableHeaderRow}>
                    ...
                    <th style={styles.tableHeader}>
                      <button
                        ...
                        title={getSortTooltip("first_name")}
                      >
                    ...
                    <th style={styles.tableHeader}>
                      ...
                      >
                        Reg Date {getSortIcon("created_at")}
                      </button>
                    </th>
                    <th style={styles.tableHeader}>
                      <button
                    ...
                    <th style={styles.tableHeader}>
                      <button
                        ...
                        title={getSortTooltip("job_title")}
                      >
                ...
                <tbody>
                  {sorted.map((user) => (
                    <tr key={user.user_id} style={styles.tableRow}>
                      ...
                      <td style={styles.tableCell}>
                        {formatDate(getDisplayDate(user))}
                      </td>
...
// ... (keep the same styles object as in the previous code)
